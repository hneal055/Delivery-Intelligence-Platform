...
// Set DEV_MACHINE_IP to your backend server LAN IP when testing on a physical device.
// iOS Simulator: 'localhost'  |  Android Emulator: '10.0.2.2'  |  Physical device: LAN IP
const DEV_MACHINE_IP = process.env.EXPO_PUBLIC_API_HOST || '192.168.12.195';
...
apiClient.interceptors.request.use(
  (config) => {
    ...
    console.log(`[API] ${config.method?.toUpperCase()} ${config.baseURL}${config.url} (timeout: ${config.timeout}ms)`);
    return config;
  },
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.code === 'ECONNABORTED') {
      ...
      handleUnauthorized(); // clears token in authStore → redirects to login
    } else {
